-- ================================================================
-- Phase 22: Analytics Views & Performance Implementation (FINAL CORRECTED)
...
-- ================================================================
-- ANALYTICS VIEWS CREATION (FINAL CORRECTED FOR ACTUAL SCHEMA)
-- ================================================================

-- ----------------------------------------------------------------
-- 1. USER ENGAGEMENT SUMMARY VIEW (Final corrected version)
...
SELECT 
    ...
    (COALESCE(session_stats.total_sessions, 0) * 2 + 
     ...
     COALESCE(env_stats.quiz_sessions, 0) * 5 + 
...
LEFT JOIN (
    SELECT 
        ...
        SUM(cf.carbon_kg) as total_carbon_kg,
        SUM(cf.carbon_saved_kg) as carbon_saved_kg
...
LEFT JOIN (
    SELECT 
        ...
        AVG(CASE WHEN qs.total_questions > 0 THEN (qs.correct_answers * 100.0 / qs.total_questions) ELSE 0 END) as avg_quiz_score
...
-- ----------------------------------------------------------------
-- 2. CONTENT PERFORMANCE VIEW (Final corrected version)
...
LEFT JOIN (
    ...
    FROM article_interactions 
    GROUP BY article_id
) interaction_stats ON a.id = interaction_stats.article_id;

-- ----------------------------------------------------------------
-- 3. ENVIRONMENTAL IMPACT SUMMARY VIEW (Final corrected version)
...
-- ----------------------------------------------------------------
-- 4. MARKETPLACE INSIGHTS VIEW (Final corrected version)
...
-- ================================================================
-- PERFORMANCE OPTIMIZATION INDEXES (FINAL CORRECTED)
-- ================================================================
...
-- ================================================================
-- REAL-TIME DASHBOARD METRICS VIEW (FINAL CORRECTED)
-- ================================================================
...
-- ================================================================
-- DATA VALIDATION VIEW (FINAL CORRECTED)
-- ================================================================
...
FROM users u 
WHERE EXISTS (SELECT 1 FROM user_sessions us WHERE us.user_id = u.id)
...
FROM articles a 
WHERE EXISTS (SELECT 1 FROM article_interactions ai WHERE ai.article_id = a.id)
...
FROM users u 
WHERE EXISTS (SELECT 1 FROM carbon_footprints cf WHERE cf.user_id = u.id)
...
-- ================================================================
INSERT INTO phase_completions (phase, status, summary, completed_at) 
VALUES (
    'Phase 22: Analytics Views & Performance Implementation (FINAL CORRECTED)', 
    'COMPLETED', 
    'Created 4 major corrected analytics views, performance indexes, real-time dashboard, and data validation system with proper schema alignment', 
    NOW()
);
...
-- ================================================================
SELECT 'Phase 22 Analytics Views Created' as status;
